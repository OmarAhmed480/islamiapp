
import 'dart:ui';

class AppColors {
static const  Color black70=Color(0xb3202020);
  static const  Color black=Color(0xff202020);
static const Color darkGray = Color(0xFF353535);
  static const  Color black60=Color(0xa8202020);
  static const  Color gold=Color(0xffE2BE7F);
  static const  Color gray=Color(0xff707070);
  static const  Color white=Color(0xffFFFFFF);
  static const  Color offWhite=Color(0xffFFF5E3);
  static const  Color brown=Color(0xff946107);
  static const  Color brownLight=Color(0xffFFD482);
static const Color darkGold = Color(0xffB19768);

 static const  Color bronze=Color(0xff856B3F);
}